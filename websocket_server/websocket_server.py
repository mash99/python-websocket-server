...
class API():
    def run_forever(self):
        try:
            ...
            print("ERROR: WebSocketsServer: line 56")#+str(e))
            exit(1)
...
class WebSocketHandler(StreamRequestHandler):
    ...
    def send_text(self, message):
        ...
        payload = message
        msg = chr(FIN | OPCODE_TEXT)
        msg += chr(len(payload))
        self.request.send(msg + payload)

        #following code works as well
#        fmt = "B"
#        payload = ''
#        for c in message:
#            payload += struct.pack(fmt, ord(c))
#        msg = struct.pack(fmt, FIN | OPCODE_TEXT)
#        msg += struct.pack(fmt, len(payload))
